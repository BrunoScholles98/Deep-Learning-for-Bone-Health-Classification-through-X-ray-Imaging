...
def show_cam_on_image(img, mask, name):
    ...
    # Redimensione a matriz heatmap para ter as mesmas dimensões da imagem
    heatmap_resized = cv2.resize(heatmap, (width, height))
    # Some as matrizes redimensionadas
    cam = heatmap_resized + np.float32(img)
    cam = cam / np.max(cam)
    cv2.imwrite("/d01/scholles/gigasistemica/gigasistemica_sandbox_scholles/heatmaps/cam{}.jpg".format(name), np.uint8(255 * cam))

class GradCam:
    def __init__(self, model, blob_name, target_layer_names, use_cuda, size):
        self.model = model
...
def show_gbs(inputs, gb_model, target_index, mask_dic):
    ...
    for idx, mask in mask_dic.items():
        ...
        # Obtenha as dimensões da imagem gb
        height_gb, width_gb, _ = gb.shape
        # Redimensione a matriz cam_mask para ter as mesmas dimensões de gb
        cam_mask_resized = cv2.resize(cam_mask, (width_gb, height_gb))
        # Multiplique as matrizes redimensionadas
        cam_gb = deprocess_image(cam_mask_resized * gb)
        cv2.imwrite('/d01/scholles/gigasistemica/gigasistemica_sandbox_scholles/heatmaps/cam_gb{}.jpg'.format(idx), cam_gb)
    cv2.imwrite('/d01/scholles/gigasistemica/gigasistemica_sandbox_scholles/heatmaps/gb.jpg', deprocess_image(gb))