...
TRAIN_NAME = 'Ateroma_CVAT_EfficientNetB0_Batch8'
BATCH_SIZE = 8
EPOCHS = 50
LOG_INTERVAL = 10
RESIZE_NUM = 4
...
# Paths
DATASET_PATH = Path('/d01/scholles/other/gigasistemica_sandbox_scholles/dataset/Ateroma_CVAT')
...
MODEL_SAVING_PATH = '/d01/scholles/other/gigasistemica_sandbox_scholles/saved_models/' + TRAIN_NAME + '.pth'
...
def get_resized_size():
    path = '/d01/scholles/other/gigasistemica_sandbox_scholles/dataset/Ateroma_CVAT/train/Ateroma'
    files = os.listdir(path)
    image = Image.open(os.path.join(path, files[0]))
    w, h = image.size
    resized = (int(w/RESIZE_NUM), int(h/RESIZE_NUM))
    return resized
